...
const actionText = a => ({ diagnose: '诊断', create_issue: '创建工单' }[a] || a)
const columns = [
...
<template>
  <div class="page">
    <header class="page-head"><h2>审计日志</h2><p>所有诊断与工单操作的可追溯记录</p></header>
    <div class="page-body">
      ...
      <a-table :columns="columns" :data-source="rows" :loading="loading" row-key="id" :pagination="{ pageSize: 15 }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'status'"><a-tag :color="record.status === 'success' ? 'green' : 'red'">{{ record.status }}</a-tag></template>
