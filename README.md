...
设计上坚持一个原则：先给出有证据支撑的辅助判断，再谈自动处置。因此诊断工具全部只读；回滚、重启、扩容等高风险操作必须人工审批，并先通过白名单、风险等级和二次确认策略。当前执行器仅做模拟验证，不调用生产接口。
...
- 告警关联分析。使用确定性的 `rule-v1` 算法在同一产品内关联 20 分钟窗口中的开放事件，以 `0.65` 为阈值生成稳定故障簇；页面展示关联依据、事件压缩率和独立事件数，并支持整簇诊断与完整事件证据快照。
- 关联人工评测。管理员为聚合事件标注真实故障组，系统按同产品事件对计算关联精确率、召回率、F1、误关联率和漏关联率；评测 JSON 同时导出算法参数、标签、事件快照与预测故障簇。
...
- 认证与权限。用户名密码登录，bcrypt 存哈希，HMAC 签名令牌携带角色与团队信息；管理员可创建账号并分配团队，服务层强制执行记忆访问隔离。
...
- 受控处置。服务端将处置动作分类为回滚、重启、扩缩容、缓存清理或流量切换，生成可预览的执行计划；未知动作、风险低报和缺少二次确认都会被阻断，执行结果结构化留档。
...
- 持久化。告警事件、关联人工标签、工单、审计、巡检、记忆、用户、审批单、诊断实验记录和回放结果均写入 MySQL，不可用时自动回退内存。
...
**首次登录**：系统启动时若数据库中没有任何用户，会自动创建管理员账号 `admin`（口令取自 `AUTH_ADMIN_PASSWORD`，默认 `admin123`），初始口令也会打印在启动日志里。登录后管理员可在“用户与团队”页面新增账号并分配团队。升级旧版本后需重新登录一次，使新令牌携带团队信息。
...
GET  /api/v1/alert-incidents?product_id=payment     查询关联故障簇与压缩指标
POST /api/v1/alert-incidents/:id/diagnose           从故障簇触发整簇诊断
GET  /api/v1/alert-correlation/evaluation           查询人工标签与成对评测指标
PUT  /api/v1/alert-correlation/labels               批量保存或清除人工标签（仅管理员）
...
POST /api/v1/memories                           新增记忆（作用域权限校验）
DELETE /api/v1/memories/:id                     删除本人或有管理权的记忆
POST /api/v1/memories/extract                   从文本提炼记忆草稿
...
GET  /api/v1/approvals/:id/execution-plan      预览受控执行计划（仅管理员）
POST /api/v1/approvals/:id/execute             二次确认并执行模拟计划（仅管理员）
...
已实现：工具调用循环、流式诊断、告警事件聚合、可解释故障簇关联、关联人工标注与成对评测、多页控制台、夜莺/Loki 适配器、数据源检测、可管理且版本化的向量 RAG、评测程序、诊断实验留档与人工标注、故障案例导入、对照回放与实验批次、资产管理（CMDB）、主动巡检、四级作用域记忆、用户与团队权限、操作人审计、高风险处置审批、策略校验与模拟执行闭环。

计划中：

- 真实环境处置适配器（当前仅运行白名单模拟执行器；接入 Kubernetes、发布平台前仍需增加环境级授权、熔断和回滚验证）；
- 更大规模、更真实的评测数据集与独立判官。
